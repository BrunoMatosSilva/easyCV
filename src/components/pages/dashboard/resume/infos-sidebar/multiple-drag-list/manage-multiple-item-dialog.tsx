...
import { IconField } from "@/src/components/ui/icon-input/field";
import { SliderField } from "@/src/components/ui/slider/field";
          ...
          {fieldType === "keywords" && (
            ...
            extraContent={value => (
              <div className="flex gap-2 flex-wrap mt-1">
                {value?.split(",").map((keywords, index) => {
                  if (!keywords.trim()) return null;

                  return <Badge key={`keyword-${index}`}>{keywords}</Badge>
                })}
              </div>
            )}
            />
